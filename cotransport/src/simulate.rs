...
pub fn run_simulate(
    ...
    if !kernel_path.exists() {
        return Err(format!("Supplied kernel_path {} does not exist", kernel_path.display()));
    }

    let mut command = Command::new(binary);
    if let Some(root_image_path) = root_image_path {
        if !root_image_path.exists() {
            return Err(format!("Supplied root_image_path {} does not exist", kernel_path.display()));
        }
        command.arg("-kernel")
...
fn determine_machine(config: &Contextualized) -> Result<Option<&'static str>, String> {
    let kernel_platform = config.sel4_config.get("KernelX86Platform").or_else(|| {
        config.sel4_config.get("KernelARMPlatform")
    })
    .ok_or_else(||
