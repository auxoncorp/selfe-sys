...
* [confignoble](confignoble) is a build dependency library that defines a seL4 configuration format (sel4.toml) and utilities for building seL4 with that config
  * Also includes a binary tool, cotransport for building seL4 applications with the help of a sel4.toml config file
* [example_application](example_application) is a Rust seL4 application that depends on libsel4-sys-gen for access to syscalls. It can be built/run independently or with `cotransport`.
